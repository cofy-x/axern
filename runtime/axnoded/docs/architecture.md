...
- ExecutionLease expiry and capability enforcement loss may only record the first terminal diagnostic and fail-stop the runtime. They retain runtime metadata and `AllocationState` until the control plane observes the terminal result and sends the final Delete with an optional output-sealing command. Only that Delete crosses the sealing barrier and releases resources; a node-local fail-stop never performs full cleanup.
