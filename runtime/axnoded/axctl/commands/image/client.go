...
func postImport(socketPath string, req importRequest, timeout time.Duration) (*importResponse, error) {
	...
	if resp.CanonicalRef == "" || resp.ImmutableRef == "" || resp.ContentDigest == "" {
		return nil, fmt.Errorf("imagemgr import response missing canonical_ref, immutable_ref, or content_digest")
	}
	return &resp, nil
}
