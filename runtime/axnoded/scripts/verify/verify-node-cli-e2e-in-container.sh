...
start_container() {
  ...
  local allocation_id
  allocation_id="$(awk -F= '/^container_id=/{print $2}' <<<"${output}")"
  printf '%s\n' "${allocation_id}"
}
...
assert_unary_exec() {
  ...
  set +e
  local unary_stdout
  unary_stdout="$(axctl --address "${AXNODED_SOCKET}" allocation exec "${allocation_id}" -- /bin/sh -c "printf '${expected_ok}\n'; printf '${expected_err}\n' >&2; exit 7" 2>"${stderr_capture}")"
  local unary_code=$?
  set -e
...
assert_interactive_exec() {
  ...
  printf 'echo %s\nexit 0\n' "${expected}" >"${stdin_path}"
  script -qec "cat '${stdin_path}' | axctl --address '${AXNODED_SOCKET}' allocation exec -it '${allocation_id}' -- /bin/sh" "${script_path}" >/dev/null
...
assert_resize_exec() {
  ...
  script -qec "stty rows 33 cols 101; axctl --address '${AXNODED_SOCKET}' allocation exec -it '${allocation_id}' -- /bin/sh -lc 'stty size; exit 0'" "${script_path}" >/dev/null
...
assert_interactive_exec_has_no_default_timeout() {
  ...
  script -qec "axctl --address '${AXNODED_SOCKET}' allocation exec -it '${allocation_id}' -- /bin/sh -lc 'sleep 12; echo ${expected}; exit 0'" "${script_path}" >/dev/null
  grep -q "${expected}" "${script_path}"
}
...
assert_wait_respects_timeout() {
  ...
  set +e
  timeout 5s axctl --address "${AXNODED_SOCKET}" --timeout 1s allocation wait "${allocation_id}" >"${timeout_capture}" 2>&1
  local wait_code=$?
  set -e

  [ "${wait_code}" -ne 124 ] || {
    echo "wait did not honor CLI timeout for ${allocation_id}" >&2
    exit 1
  }
  [ "${wait_code}" -ne 0 ] || {
    echo "wait unexpectedly succeeded for running Allocation ${allocation_id}" >&2
    exit 1
  }
...
assert_wait_reports_exit() {
  ...
  axctl --address "${AXNODED_SOCKET}" allocation wait "${allocation_id}" >"${wait_capture}"
  grep -q "^Allocation: ${allocation_id}$" "${wait_capture}"
  grep -q "^Exit Code: ${expected_exit}$" "${wait_capture}"
}

assert_force_terminate_reports_exit() {
  local allocation_id="$1"
  local wait_capture="$2"
  local expected_exit="$3"

  axctl --address "${AXNODED_SOCKET}" allocation force-terminate --reason verification-termination "${allocation_id}"
  assert_wait_reports_exit "${allocation_id}" "${wait_capture}" "${expected_exit}"
}
...
assert_list_and_inspect() {
  ...
  axctl --address "${AXNODED_SOCKET}" allocation list >"${list_capture}"
  grep -q "${allocation_id}" "${list_capture}"
  axctl --address "${AXNODED_SOCKET}" allocation inspect "${allocation_id}" >"${inspect_capture}"
  grep -q "^Allocation: ${allocation_id}$" "${inspect_capture}"
}
...
assert_diagnostics() {
  ...
  axctl --address "${AXNODED_SOCKET}" allocation diagnostics "${allocation_id}" >"${diagnostics_capture}"
  grep -q "^Allocation: ${allocation_id}$" "${diagnostics_capture}"
  ...
  axctl --address "${AXNODED_SOCKET}" allocation diagnostics --json "${allocation_id}" >"${diagnostics_json_capture}"
  jq -e '
...
[ -n "${runsc_id}" ] || {
  echo "verify-cli did not return a runsc Allocation id" >&2
  exit 1
}
...
assert_force_terminate_reports_exit "${runsc_id}" /tmp/runsc.force-terminate.wait.output 137
...
[ -n "${runsc_wait_id}" ] || {
  echo "verify-cli did not return a second runsc Allocation id" >&2
  exit 1
}
assert_wait_reports_exit "${runsc_wait_id}" /tmp/runsc.wait.output 11
axctl --address "${AXNODED_SOCKET}" allocation force-cleanup --reason verification-cleanup "${runsc_wait_id}"
