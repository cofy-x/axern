...
NODE_CONTAINER_NAME="${NODE_CONTAINER_NAME:-axnoded-node-inventory-e2e}"
...
case "${VERIFY_DOCKER_PLATFORM}" in
  ...
  linux/arm64 | linux/arm64/v8)
    ...
    ;;
  *)
    echo "unsupported VERIFY_DOCKER_PLATFORM for inventory e2e: ${VERIFY_DOCKER_PLATFORM}" >&2
    exit 1
    ;;
esac
...
if ! docker exec \
  ...
  "${NODE_CONTAINER_NAME}" \
  /bin/bash /workspace/scripts/verify/verify-node-inventory-e2e-in-container.sh; then
  dump_logs
  exit 1
fi

echo "verify_node_inventory_e2e_host_ok=true"
