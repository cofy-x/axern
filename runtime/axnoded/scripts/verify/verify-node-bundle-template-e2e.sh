...
NODE_CONTAINER_NAME="${NODE_CONTAINER_NAME:-axnoded-node-bundle-template-e2e}"
AXNODED_IDLE_ENVIRONMENT_RETENTION_TTL="${AXNODED_IDLE_ENVIRONMENT_RETENTION_TTL:-30s}"
...
docker run -d \
  ...
  -e "AXNODED_IDLE_ENVIRONMENT_RETENTION_TTL=${AXNODED_IDLE_ENVIRONMENT_RETENTION_TTL}" \
  -e "AXNODED_IDLE_ENVIRONMENT_RETENTION_MAX=${AXNODED_IDLE_ENVIRONMENT_RETENTION_MAX}" \
  "${IMAGE_TAG}" \
...
if ! docker exec \
  -e "AXNODED_SOCKET=${AXNODED_SOCKET}" \
  "${NODE_CONTAINER_NAME}" \
  /bin/bash /workspace/scripts/verify/verify-node-bundle-template-e2e-in-container.sh; then
  dump_logs
  exit 1
fi

echo "verify_node_bundle_template_e2e_host_ok=true"
