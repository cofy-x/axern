...
func TestScenario1_GetNotBlockedBySlowAdd(t *testing.T) {
	...
	for i := 0; i < preRegistered; i++ {
		go func(idx int) {
			defer setupWg.Done()
			addTestEnvironmentCache(lm, newTestFR(fmt.Sprintf("existing-%d", idx), fmt.Sprintf("/existing/%d", idx)))
		}(i)
	}
	setupWg.Wait()
	...
	for i := 0; i < numAdders; i++ {
		go func(idx int) {
			defer adderWg.Done()
			addTestEnvironmentCache(lm, newTestFR(
	...
	for i := 0; i < numGetters; i++ {
		go func(idx int) {
			...
			start := time.Now()
			environment := lm.GetPreparedEnvironment(id)
			latency := time.Since(start)
			if environment == nil {
				t.Errorf("GetPreparedEnvironment(%s) returned nil", id)
				return
			}
			for {
...
func TestScenario2_DifferentRootfsParallel(t *testing.T) {
	...
	for i := 0; i < numAdders; i++ {
		go func(idx int) {
			...
			if _, err := addTestEnvironmentCache(lm, newTestFR(id, path)); err != nil {
				t.Errorf("PrepareEnvironment %s failed: %v", id, err)
			}
		}(i)
	}
	wg.Wait()
...
func TestScenario3_SameRootfsSingleMount(t *testing.T) {
	...
	var wg sync.WaitGroup
	wg.Add(numAdders)
	results := make([]*PreparedEnvironment, numAdders)
	...
	for i := 0; i < numAdders; i++ {
		go func(idx int) {
			defer wg.Done()
			id := fmt.Sprintf("rt-%d", idx)
			results[idx], errs[idx] = addTestEnvironmentCache(lm, newTestFR(id, "/shared/rootfs"))
		}(i)
	}
	wg.Wait()
	elapsed := time.Since(start)
...
func TestHighConcurrencyMixed(t *testing.T) {
	...
	for i := 0; i < preRegistered; i++ {
		addTestEnvironmentCache(lm, newTestFR(fmt.Sprintf("pre-%d", i), fmt.Sprintf("/pre/%d", i)))
	}
	...
	for i := 0; i < numAdders; i++ {
		go func(idx int) {
			...
			addTestEnvironmentCache(lm, newTestFR(id, path))
			totalOps.Add(1)
		}(i)
	}
	...
	for i := 0; i < numGetters; i++ {
		go func(idx int) {
			defer wg.Done()
			lm.GetPreparedEnvironment(fmt.Sprintf("pre-%d", idx%preRegistered))
			totalOps.Add(1)
		}(i)
	}
