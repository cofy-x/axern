...
func TestExecutionLeaseExpiryTerminationIntentClassifiesRuntimeExit(t *testing.T) {
	...
	if err := s.allocations.StoreAllocationIntent(allocationID, "node-a", digest, time.Now().Add(time.Minute), nil, nil, nil, nil); err != nil {
		t.Fatal(err)
	}
...
func TestTerminalCheckpointSeedsDurableOutboxBeforeContainerCleanup(t *testing.T) {
	...
	if err := allocationController.StoreAllocationIntent("alloc-recovered", "node-a", "sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", time.Now().Add(time.Minute), nil, nil, nil, nil); err != nil {
