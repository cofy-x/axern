...
func (BridgeNetworkManager) SetupDNATRule(protocol string, dstPort uint16, targetIP string, targetPort uint16) error {
	return setupDNATRule(protocol, dstPort, targetIP, targetPort)
}

func setupDNATRule(protocol string, dstPort uint16, targetIP string, targetPort uint16) error {
...
func (BridgeNetworkManager) CleanupDNATRule(protocol string, dstPort uint16, targetIP string, targetPort uint16) error {
	return cleanupDNATRule(protocol, dstPort, targetIP, targetPort)
}

func cleanupDNATRule(protocol string, dstPort uint16, targetIP string, targetPort uint16) error {
	ipt, ipv6, err := iptablesForAddress(targetIP)
	if err != nil {
		return err
	}
