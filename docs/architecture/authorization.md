...
External model access keeps four authorities distinct. The runner's Axern mTLS credential authenticates its Principal to control APIs; a one-time Tunnel client token admits only one peer to one finite TunnelSession; an optional runner-local gateway token scopes one Allocation/episode/model/budget at the application layer; and the Provider API key or client certificate remains solely in that runner-local gateway. None substitutes for an AllocationAccessGrant or ExecutionLease, and none may be copied into Run metadata, sandbox configuration, output bundles, URLs, logs, or diagnostics. Axern transports raw TCP and does not interpret or persist model-provider protocols or credentials.
