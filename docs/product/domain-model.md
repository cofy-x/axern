...
Stdout, stderr, and declared outputs remain Allocation-local. When infrastructure cleanup begins, the control-plane transaction freezes `output_expires_at` at 15 minutes after that transition. Before deleting the runtime, the node stops the supervised workload while retaining sandboxd as OCI PID 1, then atomically publishes one read-only manifest through its still-live file/archive service. Runtime cleanup cannot cross that publication barrier; a failed capture retries under the same cleanup intent. ExecutionLease expiry or capability loss may stop the workload but cannot remove sandboxd, runtime metadata, or `AllocationState`. Reads survive runtime cleanup and node-process restart until the deadline, but not node-disk loss.
