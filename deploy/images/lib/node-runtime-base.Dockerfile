...
RUN set -eux; \
    ...
    for COMPONENT in checkpointgofer gvisor_sentry runsc-fd-parking; do test -x "/tmp/gvisor/gvisor-bin/$COMPONENT"; done; \
    mkdir -p /usr/local/bin/gvisor-bin; \
    cp -a /tmp/gvisor/gvisor-bin/. /usr/local/bin/gvisor-bin/; \
