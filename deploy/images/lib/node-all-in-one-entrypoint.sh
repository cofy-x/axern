...
AXNODED_ROOTFS_SNAPSHOT_REPOSITORY="${AXNODED_ROOTFS_SNAPSHOT_REPOSITORY:-}"
...
AXNODED_IDLE_ENVIRONMENT_RETENTION_TTL="${AXNODED_IDLE_ENVIRONMENT_RETENTION_TTL:-5m}"
AXNODED_IDLE_ENVIRONMENT_RETENTION_MAX="${AXNODED_IDLE_ENVIRONMENT_RETENTION_MAX:-8}"
...
AXNODED_CONTROL_PLANE_ENROLLMENT_TARGET="${AXNODED_CONTROL_PLANE_ENROLLMENT_TARGET:-}"
...
AXNODED_CONFIG="${AXNODED_ROOT}/node-config.toml"
...
install -m 0600 /dev/null "${AXNODED_CONFIG}"
...
control_plane_enrollment_target = "${AXNODED_CONTROL_PLANE_ENROLLMENT_TARGET}"
...
rootfs_snapshot_repository = "${AXNODED_ROOTFS_SNAPSHOT_REPOSITORY}"
...
ephemeral_storage_default_limit_bytes = ${AXNODED_EPHEMERAL_STORAGE_DEFAULT_LIMIT_BYTES}
idle_environment_retention_ttl = "${AXNODED_IDLE_ENVIRONMENT_RETENTION_TTL}"
idle_environment_retention_max = ${AXNODED_IDLE_ENVIRONMENT_RETENTION_MAX}
...
/usr/local/bin/imagemgr \
  ...
  -registry_auths_path "${REGISTRY_AUTHS}" \
	-registry_mirror_url "${REGISTRY_MIRROR_URL}" \
	-snapshot_repository "${AXNODED_ROOTFS_SNAPSHOT_REPOSITORY}" \
  ...
  if curl -fsS "http://127.0.0.1:23001/readyz" >/dev/null 2>&1; then
    if [ "${NODE_TUNNELD_ENABLED}" = "true" ] && [ -n "${AXNODED_CONTROL_PLANE_TARGET}" ] && [ -n "${AXNODED_CONTROL_PLANE_NODE_ID}" ]; then
      node_tunneld_args=(
        ...
        -identity-bundle "${AXNODED_ROOT}/root/identity/node.pem" \
        -workload-cluster "${AXERN_WORKLOAD_CLUSTER}" \
