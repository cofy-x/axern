...
Keep `node.network.ebpf.localOutCompat=true` and `node.network.ebpf.iptablesFallback=true`. A mode that includes `localhost-tcp-iptables-compat` is acceptable when the kernel cannot expose the localhost TCP cgroup path, as long as TC ingress and egress remain attached. `iptables-full-fallback` is not a successful bpfnet replacement state.
