...
- [Control Plane](./controld/README.md): Postgres-backed daemon for node registration, heartbeat ingest, lifecycle control, gateway route resolution, execution leases, and the read-only environment template set. The control plane owns durable product metadata, placement, admission, lifecycle APIs, gateway route resolution, node image inventory summaries, and revocable execution leases. Rootfs locality describes local directories or registry images (OCI/Nydus). Realtime exec, file/archive transfer, terminal, SSH, and tunnel traffic stay outside the control plane and flow through `gatewayd`.
