...
func loadMigrations() ([]Migration, error) {
	...
	for _, entry := range entries {
		...
		if !ok {
			return nil, fmt.Errorf("invalid postgres migration file name %q", entry.Name())
		}
		if prior, exists := seen[version]; exists {
			return nil, fmt.Errorf("duplicate postgres migration version %06d: %s and %s", version, prior, entry.Name())
		}
	...
	if len(migrations) == 0 {
		return nil, fmt.Errorf("no postgres migrations embedded")
	}
