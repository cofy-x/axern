...
func (s *Store) activeRunInventoryExpectations(ctx context.Context, nodeID string) ([]allocationkernel.NodeInventoryExpectation, error) {
	rows, err := s.db.Pool().Query(ctx, `
		SELECT a.allocation_id, a.node_active_at
		FROM allocations a
		WHERE a.node_id = $1 AND a.lifecycle_state IN ($2, $3)
		ORDER BY a.created_at ASC, a.allocation_id ASC
	`, strings.TrimSpace(nodeID), commonv1.AllocationLifecycleState_ALLOCATION_LIFECYCLE_STATE_STARTING.String(), commonv1.AllocationLifecycleState_ALLOCATION_LIFECYCLE_STATE_ACTIVE.String())
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&record.AllocationID, &nodeActiveAt); err != nil {
...
func (s *Store) unavailableRunAllocations(ctx context.Context, nodeID string) ([]allocationkernel.NodeAllocationRef, error) {
	rows, err := s.db.Pool().Query(ctx, `
		...
		FROM allocations a
		WHERE a.node_id = $1 AND a.lifecycle_state IN ($2, $3, $4)
		ORDER BY a.created_at ASC, a.allocation_id ASC
	...
	for rows.Next() {
		...
		if err := rows.Scan(&record.AllocationID); err != nil {
