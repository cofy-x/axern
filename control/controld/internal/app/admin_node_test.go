...
func TestPostgresAdminRetiresIdleNodeAndFencesReporter(t *testing.T) {
	...
	_, err = app.NodeV1Handler().ReportNode(context.Background(), &nodev1.ReportNodeRequest{NodeID: "node-a", NodeTarget: "127.0.0.1:25000", NodeCredential: testNodeCredential, Summary: controldtest.ReadySummary(now.Add(2 * time.Hour))})
