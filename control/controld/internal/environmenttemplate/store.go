...
var runtimeImageOverrideEnv = map[string]string{
	"python311":    "AXERN_RUNTIME_TEMPLATE_PYTHON311_IMAGE",
	"server-base":  "AXERN_RUNTIME_TEMPLATE_SERVER_BASE_IMAGE",
	"coding-base":  "AXERN_RUNTIME_TEMPLATE_CODING_BASE_IMAGE",
	"desktop-base": "AXERN_RUNTIME_TEMPLATE_DESKTOP_BASE_IMAGE",
}

type Store struct {
	environmentTemplates map[string]*privateenvironmentv1.EnvironmentTemplate
}

func NewStore(in []*privateenvironmentv1.EnvironmentTemplate) *Store {
	if len(in) == 0 {
		in = DefaultTemplates()
	}
...
func (s *Store) List() []*privateenvironmentv1.EnvironmentTemplate {
	...
	out := make([]*privateenvironmentv1.EnvironmentTemplate, 0, len(keys))
	for _, key := range keys {
		template := s.environmentTemplates[key]
		out = append(out, proto.Clone(template).(*privateenvironmentv1.EnvironmentTemplate))
	}
	return out
}

func (s *Store) Get(id, version string) (*privateenvironmentv1.EnvironmentTemplate, bool) {
	...
	if version = strings.TrimSpace(version); version != "" {
		...
		return proto.Clone(template).(*privateenvironmentv1.EnvironmentTemplate), true
	}
	candidates := make([]*privateenvironmentv1.EnvironmentTemplate, 0)
	...
	return proto.Clone(candidates[0]).(*privateenvironmentv1.EnvironmentTemplate), true
}

func DefaultTemplates() []*privateenvironmentv1.EnvironmentTemplate {
...
func loadDefaultTemplates() ([]*privateenvironmentv1.EnvironmentTemplate, error) {
...
func parseDefaultTemplates(data []byte) ([]*privateenvironmentv1.EnvironmentTemplate, error) {
	...
	templates := make([]*privateenvironmentv1.EnvironmentTemplate, 0, len(rawTemplates))
	seen := make(map[string]struct{}, len(rawTemplates))
	for idx, raw := range rawTemplates {
		template := &privateenvironmentv1.EnvironmentTemplate{}
...
func validateDefaultEnvironmentTemplate(idx int, template *privateenvironmentv1.EnvironmentTemplate, seen map[string]struct{}) error {
	if template.GetID() == "" {
...
func applyEnvironmentTemplateOverrides(template *privateenvironmentv1.EnvironmentTemplate) {
	if template == nil {
		return
	}
	...
	if template.ResolvedSpec == nil {
		template.ResolvedSpec = &environmentv1.ResolvedEnvironmentSpec{}
	}
	if template.ResolvedSpec.ImageDescriptor == nil {
		template.ResolvedSpec.ImageDescriptor = &environmentv1.OciImageDescriptor{}
	}
...
func cloneEnvironmentTemplateMap(in []*privateenvironmentv1.EnvironmentTemplate) map[string]*privateenvironmentv1.EnvironmentTemplate {
	out := make(map[string]*privateenvironmentv1.EnvironmentTemplate, len(in))
	for _, template := range in {
		...
		out[templateKey(id, template.GetVersion())] = proto.Clone(template).(*privateenvironmentv1.EnvironmentTemplate)
	}
	return out
}
