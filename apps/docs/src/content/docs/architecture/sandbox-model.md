...
1. Resolve or create an **Environment** (an OCI image or existing environment ID — exactly one source).
...
Each Sandbox selects exactly one source: a portable OCI `image` or an existing `environment_id` to continue prior work.
...
Public RPC errors preserve the operation, RPC code, server details, retryability, and allocation identity. Validation, not-found, permission, timeout, cancellation, and unavailable failures stay distinct. SDKs never retry mutating RPCs implicitly; idempotent reads and Run-watch reconnects retry only within the caller's deadline.

The authoritative contract is the repository's [SDK user model](https://github.com/cofy-x/axern/blob/main/docs/product/sdk-user-model.md).
