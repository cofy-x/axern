...
The catalog is the control plane's registry of runtime templates and agent bundles. A template defines a sandbox rootfs; a bundle defines a read-only agent or tool image mounted into a sandbox.
...
Axrun resolves the matching `codex` or `claude-code` Agent Bundle and freezes its digest at rollout planning time.
