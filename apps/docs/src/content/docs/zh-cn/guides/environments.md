...
Environment 是不可变、可复用的执行源：解析好的 OCI 镜像引用，Run 和 Sandbox 可以共享它而不必重复解析镜像。命名空间组织资源，配额限制各命名空间可准入的用量。
...
环境必须提供一个明确的 `--image-ref`。私有仓库使用按 ID 引用的已存凭据：
...
环境是不可变的——镜像变更意味着创建新环境。Run 准入时会把规范化 source 与已解析运行时输入冻结到 Run。删除可复用 Environment 会将其从后续查询和准入中物理移除，但不会改变或破坏已经准入的 Run。
