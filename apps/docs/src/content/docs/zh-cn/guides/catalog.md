...
`python311`、`coding-base` 等模板为工作负载提供可复现的、平台策展的 rootfs。每个 Run、Service 和 SDK Sandbox 严格选择一个 source：Catalog 模板、通用 OCI 镜像或已有环境。简单试验优先用通用 OCI 镜像；在模板的 Catalog 与复用语义真正重要的地方再引入模板。
