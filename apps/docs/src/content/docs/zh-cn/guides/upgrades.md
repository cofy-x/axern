...
Axern 的 CLI、Helm Chart、运行时镜像和三个 SDK 以同一仓库版本发布。每个 Release 都应视为一个整体：混用版本不是受支持的组合。

## 升级前检查

更改任何组件前先阅读目标版本的[发布说明](https://github.com/cofy-x/axern/tree/main/docs/releases)。发布说明定义状态能否保留、哪些契约发生变化，以及是否必须全新部署。替换状态前先导出所需输出，并确认工作负载和清理已经收敛。不要删除仍在运行的 sandbox 所依赖的节点恢复记录，也不要在发布完成前使用候选版本。
...
目标 Release 明确支持保留现有状态时，把 Chart 锁定到与 CLI 相同的版本，并根据新 Chart 审查运维自持的 values。不要沿用旧镜像标签或已删除的配置。如果目标 Release 不支持状态保留，应先导出所需输出、停止工作负载、确认清理完成，再按照该 Release 的全新部署流程操作，而不是原地升级。
...
公共兼容性由目标 Release 的发布说明定义，而不是由内部数据库表、Proto package 或节点本地文件定义。跨版本升级时阅读仓库 [`docs/releases/`](https://github.com/cofy-x/axern/tree/main/docs/releases) 中的发布说明，并在恢复工作负载前对升级后的平台运行 `axern doctor --namespace default`。
