...
`spec.source` 在 `image`、`template` 和 `environment` 中严格三选一。`readiness` 和 `liveness` 接受 `http` 探针（`port`、`path`、`scheme`）或 `tcp_port`，以及 `initial_delay`、`period`、`timeout`、`success_threshold`、`failure_threshold` 时长。打包节点使用 `runsc` 运行服务；见[运行时与资源](/zh-cn/architecture/resources/)。
