...
| hostPort 回复的源地址恢复到节点 hostPort   | TC egress                                         |
...
`axnoded` 掌管 Sandbox 生命周期、bridge/veth/netns 资源、hostPort 端点意图、后端选择、回滚策略和 SNAT GC 调度。`bpfnet` 掌管数据面挂接与 reconcile、service map 编程、固定的 map 和程序，以及状态采集。`bpfnetctl` 是只读诊断工具，从不写入端点意图。
