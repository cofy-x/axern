...
func (m *Manager) writeEnv(profile string) error {
	...
	for _, key := range []string{"postgres", "dev_token", "enrollment_token"} {
		if secretValues[key] == "" {
	...
	values := map[string]string{
		...
		"OTEL_COLLECTOR_IMAGE": images["OTEL_COLLECTOR_IMAGE"], "OTEL_LGTM_IMAGE": images["OTEL_LGTM_IMAGE"], "AXERN_SECRETS_MASTER_KEY": secretValues["master"], "LOCAL_DEV_TOKEN": secretValues["dev_token"], "ENROLLMENT_TOKEN": secretValues["enrollment_token"],
