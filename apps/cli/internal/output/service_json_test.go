...
func TestPrintServiceResponseJSONIncludesDeletionStatus(t *testing.T) {
	...
	if deletion.Phase != "releasing-allocations" || deletion.Message != "waiting for allocation cleanup" || deletion.CompletedAt != "2026-08-13T08:30:00Z" {
		t.Fatalf("deletion_status = %#v, want stable deletion projection", deletion)
	}
	for _, unexpected := range []string{"SERVICE_DELETION_PHASE"} {
		if strings.Contains(b.String(), unexpected) {
