...
The image-ref smoke uses `docker.io/library/nginx:1.27` with `runsc`
by default and can be pointed at another external image with
`AXERN_CLI_E2E_IMAGE_REF`. Override the runtime matrix with
`AXERN_CLI_E2E_IMAGE_REF_RUNTIME_CLASSES` only against a node explicitly
configured for that diagnostic matrix; packaged nodes enable runsc only.
To use a proxy, export standard
