...
bundle_build_dir="$(mktemp -d "${TMPDIR:-/tmp}/axern-image-mount-smoke.XXXXXX")"
cleanup_bundle_build_dir() {
  rm -rf "${bundle_build_dir}"
}
trap 'cleanup_bundle_build_dir; end_env_lock compose' EXIT

cat >"${bundle_build_dir}/hello-tool" <<'EOF'
#!/bin/sh
echo image-mount-smoke-ok
EOF
chmod +x "${bundle_build_dir}/hello-tool"
cat >"${bundle_build_dir}/Dockerfile" <<'EOF'
FROM scratch
COPY hello-tool /bin/hello-tool
EOF

bundle_source_image="axern/image-mount-smoke-tool:local"
docker build -q -t "${bundle_source_image}" "${bundle_build_dir}" >/dev/null

cat >"${bundle_build_dir}/Task.Dockerfile" <<'EOF'
ARG TASK_BASE_IMAGE
FROM ${TASK_BASE_IMAGE}
RUN mkdir -p /opt/axern/tools/hello
EOF

task_source_image="axern/image-mount-smoke-task:local"
docker build -q --build-arg "TASK_BASE_IMAGE=${task_base_image}" -f "${bundle_build_dir}/Task.Dockerfile" -t "${task_source_image}" "${bundle_build_dir}" >/dev/null
...
dump_image_mount_smoke_failure() {
  if [ -n "${run_id}" ]; then
...
cleanup_image_mount_smoke() {
  local rc=$?
  if [ "${rc}" -ne 0 ]; then
    dump_image_mount_smoke_failure || true
  fi
  if [ -n "${run_id}" ]; then
...
create_output="$(
  local_smoke_json_once_or_recover_by_namespace run runs run "${namespace}" \
    "${AXERN_SMOKE_CMD[@]}" run create -o json --wait --wait-for terminal --wait-timeout 180s \
      ...
      --argv /bin/sh --argv -lc \
      --argv '/opt/axern/tools/hello/bin/hello-tool | grep -Fx image-mount-smoke-ok && ! touch /opt/axern/tools/hello/write-test' || true
)"
