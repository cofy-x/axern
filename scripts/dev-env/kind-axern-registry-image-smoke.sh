...
namespace="axern-registry-image-smoke-$(date +%s)"
run_id=""
environment_id=""
...
dump_registry_image_smoke_failure() {
  ...
  for pod in $(kubectl -n "${K8S_NAMESPACE}" get pods -l app=node-all-in-one -o jsonpath='{range .items[*]}{.metadata.name}{"\n"}{end}' 2>/dev/null); do
    echo "--- ${pod}: imagemgr log tail ---" >&2
    kubectl -n "${K8S_NAMESPACE}" exec "${pod}" -- tail -n 120 /var/lib/imagemgr/logs/imagemgr.log >&2 || true
    echo "--- ${pod}: axnoded log tail ---" >&2
    kubectl -n "${K8S_NAMESPACE}" exec "${pod}" -- tail -n 120 /var/log/axnoded/axnoded.log >&2 || true
  done
}
...
echo "kind_axern_registry_image_smoke_ok=true"
