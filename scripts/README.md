...
- `dev-env/compose-dns-doctor-smoke.sh` Verifies the materialized Compose resolver configuration through the internal Node probe and the public `axern local doctor` config, Node, and real OCI sandbox layers. The enclosing refresh requires the caller to provide `AXERN_VERIFY_DNS_NAMESERVERS`; it never guesses a resolver or falls back to a public service.
