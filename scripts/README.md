...
- `dev-env/compose-dns-doctor-smoke.sh` Verifies axnoded's effective Compose resolver configuration through the internal Node probe and the public `axern local doctor` config, Node, and real OCI sandbox layers. The enclosing refresh derives the Node resolver by default; `AXERN_VERIFY_DNS_NAMESERVERS` is an optional deterministic override, and no path falls back to a public service.
