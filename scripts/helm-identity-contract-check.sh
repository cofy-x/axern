...
for name in ("worker-a", "worker-b"):
    documents = [doc for doc in nodes if f'- key: "node-{name}"' in doc]
    assert len(documents) == 1, f"missing isolated enrollment token for {name}"
    doc = documents[0]
    other = "worker-b" if name == "worker-a" else "worker-a"
    assert f"node-{other}" not in doc, "another Node token leaked"
    ...
    assert token_volume.count("- key:") == 1 and "defaultMode: 384" in token_volume
...
duplicate = subprocess.run(args[:-2] + ["--set-json", 'node.enrollment.nodes=["worker-a","worker-a"]'],
