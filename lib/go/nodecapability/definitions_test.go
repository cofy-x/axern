...
func TestDefinitionsAreCompleteAndAcyclic(t *testing.T) {
...
func TestDefinitionsRejectInvalidDependencyAndLossPolicy(t *testing.T) {
	...
	definitionSet := cloneDefinitions()
	derived := definitionSet[capabilityv1.PlatformCapability_PLATFORM_CAPABILITY_RUNSC_MEMORY_HARD_LIMIT]
	derived.Dependencies = []capabilityv1.PlatformCapability{derived.Key}
	definitionSet[derived.Key] = derived
	if err := validateDefinitions(definitionSet); err == nil {
		t.Fatal("definitions accepted a dependency cycle")
	}
	definitionSet = cloneDefinitions()
	withoutVerifier := definitionSet[capabilityv1.PlatformCapability_PLATFORM_CAPABILITY_ROOTFS_LOWER_EROFS]
	withoutVerifier.LossPolicy = capabilityv1.CapabilityLossPolicy_CAPABILITY_LOSS_POLICY_FAIL_STOP
	definitionSet[withoutVerifier.Key] = withoutVerifier
	if err := validateDefinitions(definitionSet); err == nil {
		t.Fatal("definitions accepted fail-stop without an allocation verifier")
	}
}

func TestAvailableObservationUsesDefinitionDependenciesAndFailsClosed(t *testing.T) {
	...
	if _, ok := AvailableObservation(snapshot, derivedKey, now); ok {
		t.Fatal("derived requirement survived an unavailable definition dependency")
	}
	base = testObservation(baseKey, now)
