// Package nodecapability owns the canonical node capability definition registry and the
...
var definitions = mustDefinitions(map[capabilityv1.PlatformCapability]Definition{
	...
	capabilityv1.PlatformCapability_PLATFORM_CAPABILITY_ROOTFS_SNAPSHOT:                       platformDefinition(capabilityv1.PlatformCapability_PLATFORM_CAPABILITY_ROOTFS_SNAPSHOT, capabilityv1.CapabilityProvider_CAPABILITY_PROVIDER_CONFIG, IdentityConfig, 0, capabilityv1.CapabilityLossPolicy_CAPABILITY_LOSS_POLICY_ADMISSION_ONLY, AudienceWorkloadRequirement, VerifierNone),
})
...
func mustDefinitions(in map[capabilityv1.PlatformCapability]Definition) map[capabilityv1.PlatformCapability]Definition {
	if err := validateDefinitions(in); err != nil {
		panic("invalid capability definition registry: " + err.Error())
	}
	return in
}

func validateDefinitions(in map[capabilityv1.PlatformCapability]Definition) error {
